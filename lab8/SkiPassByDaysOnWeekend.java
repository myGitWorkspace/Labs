...
/** Daughter class of SkiPass that counts days of SkiPass card usage only on Weekend
 * Controls correct hours of usage when half of day is selected
 * 
 *
 */
public class SkiPassByDaysOnWeekend extends SkiPass  {
	
	private ArrayList<Integer> daysAllowed = new ArrayList<Integer>();
	private int[] daysNumb = new int[] {1,1,2};
	private int daysNumbId;
	private Calendar lastDateLoginSkiPass;
	
	public SkiPassByDaysOnWeekend(int id, Calendar expireDate, int daysNumbId) {
		super(id, expireDate);
	...
	public boolean loginCheckCard(Calendar currDate) {
		...
		if ( !daysAllowed.contains( currDate.get(Calendar.DAY_OF_WEEK) ) ) {
			System.out.println("Access forbiden!!! You can use you card only on Saturday or on Sunday");
			return false;
		}
